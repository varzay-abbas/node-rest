...
class ClinicRepository {
                ...
                await axios.get(url1)
                    .then(function (response) {
                        ...
                        for (let x in res) {
                            ...
                            item["Availability"] = "from:" + res[x].availability["from"] + ", to:" + res[x].availability["to"];
                            jsonObj.push(item);
                ...
                await axios.get(url2)
                    .then(function (response) {
                        ...
                        for (let x in res) {
                            ...
                            item["Availability"] = "from:" + res[x].opening["from"] + ", to:" + res[x].opening["to"];
                            jsonObj.push(item);
                        }
                    })
                    .catch(function (error) {
    ...
    searchClinicData(ClinicName, State, Availability) {
        ...
        if (ClinicName)
            filtered = _.where(filtered, { "ClinicName": ClinicName });

        if (State)
            filtered = _.where(filtered, { "State": State });
